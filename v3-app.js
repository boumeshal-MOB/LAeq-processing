(function(){
  ...
  $('fmode').value = state.frequencyMode || 'auto';
  ...
  function renderOutputs(){
    ...
    outputs.forEach((o,idx)=>{
      ...
      tr.querySelectorAll('[data-k]').forEach(input=>input.oninput=()=>{let v=input.value; if(['duration','step'].includes(input.dataset.k))v=Number(v)||1; if(input.dataset.k==='active')v=v==='true'; o[input.dataset.k]=v; renderOutputs(); updateScheduler(); persist();});
      tr.querySelector('[data-remove]').onclick=()=>{outputs=outputs.filter(x=>x.id!==o.id); renderOutputs(); updateScheduler(); persist();};
  ...
  function updateScheduler(){
    ...
    $('recommended').textContent = rec.seconds ? `${C.duration(rec.seconds)} (${C.trim(rec.seconds/60)} min)` : '—';
    $('reason').textContent = rec.reason || 'Load source data and activate at least one output.';
    const selected = $('fmode').value==='auto' ? rec.seconds : (Number($('fcustom').value)||0)*60;
    const warnings=[...(rec.warnings||[])];
    if(rec.seconds && selected>rec.seconds) warnings.push(`Custom frequency is slower than recommended. Results may be delayed by up to ${C.duration(selected-rec.seconds)}, but catch-up will calculate all complete missing periods.`);
    if(acquisitionSec && selected && selected<acquisitionSec) warnings.push(`Custom frequency is faster than source acquisition (${C.duration(acquisitionSec)}). Some executions will have no new value.`);
    $('warning').innerHTML = warnings.length ? `<div class="warn">${warnings.map(C.esc).join('<br>')}</div>` : '<div class="ok">Scheduler frequency is consistent with the active outputs.</div>';
  ...
  $('add').onclick=()=>{outputs.push({id:'out'+Date.now(),variableName:'Noise_LAeq_custom',displayName:'LAeq custom',duration:30,unit:'min',mode:'fixed',step:30,stepUnit:'min',calendarStart:'07:00',calendarEnd:'17:00',active:true});renderOutputs();updateScheduler();persist();};
  ['fmode','fcustom','coverage','late','catchup','schedActive','active','pname'].forEach(id=>{$(id).oninput=()=>{$('fcustom').disabled=$('fmode').value!=='custom';updateScheduler();persist();};});
  ...
  function processingRows(){
    const currentFrequency = currentFrequencyLabel();
    return [
      {id:'current',name:$('pname').value,source:currentVariableName(),outputs:outputs.length,frequency:currentFrequency,last:state.lastExecution?new Date(state.lastExecution).toLocaleString():'Never',status:$('active').value==='No'?'Inactive':state.lastExecution?'Healthy':'Never executed'},
      {id:'sample1',name:'Tarmac quarry noise',source:'CUBE / LAeq',outputs:2,frequency:'15 min',last:'2026-07-10 11:00',status:'Healthy'},
      {id:'sample2',name:'Night-time monitoring',source:'Micromate / Mic Leq',outputs:1,frequency:'1 h',last:'Waiting',status:'Waiting for data'}
    ];
  }
  function currentFrequencyLabel(){const rec=C.recommendedFrequency(outputs,acquisitionSec).seconds;const s=$('fmode').value==='auto'?rec:(Number($('fcustom').value)||0)*60;return C.duration(s);}
  ...
  function showDetails(id){
    ...
    box.innerHTML=`<div class="details-head"><div><h3>Edit — ${C.esc($('pname').value)}</h3><div class="help">Full configuration appears only after Edit: generated variables, scheduler, catch-up state and history.</div></div><div><button id="editOutputs" class="btn small">Edit outputs</button> <button id="manualFromAdmin" class="btn small yellow">Manual run</button> <button id="clearState" class="btn small danger">Reset state</button></div></div><div class="stats"><div class="stat"><span>Source</span><b>${C.esc(currentVariableName())}</b></div><div class="stat"><span>Frequency</span><b>${C.esc(currentFrequencyLabel())}</b></div><div class="stat"><span>Last run</span><b>${state.lastExecution?new Date(state.lastExecution).toLocaleString():'Never'}</b></div><div class="stat"><span>Status</span><b>${$('active').value==='No'?'Inactive':state.lastExecution?'Healthy':'Never executed'}</b></div></div><h3 class="section-title">Generated variables</h3><div class="tw"><table><thead><tr><th>Variable</th><th>Calculation</th><th>Expected values</th><th>Last calculated end</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></div><h3 class="section-title">Execution history</h3><div class="tw"><table><thead><tr><th>Time</th><th>Trigger</th><th>Results</th><th>Outputs checked</th><th>Status</th></tr></thead><tbody>${hist}</tbody></table></div>`;
  ...
  renderOutputs();updateScheduler();renderAdministration();
})();
